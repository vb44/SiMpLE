...
{
    std::cerr << "scancontext lib is successfully imported." << endl;
} // coreImportTest
...
std::pair<int, float> SCManager::detectLoopClosureID ( void )
{
    ...
    if( min_dist < SC_DIST_THRES )
    {
        ...
        *logFile_ << "[Loop found] Nearest distance: " << min_dist << " btn " << polarcontexts_.size()-1 << " and " << nn_idx << "." << std::endl;
        // cout << "[Loop found] yaw diff: " << nn_align * PC_UNIT_SECTORANGLE << " deg." << endl;
    }
    else
    {
        // std::cerr.precision(3); 
        *logFile_ << "[Not loop] Nearest distance: " << min_dist << " btn " << polarcontexts_.size()-1 << " and " << nn_idx << "." << std::endl;
...
void SCManager::setLogFile(std::ofstream& logFile)
{
    logFile_ = &logFile;
}
